...
from
    (
        ...
        where dt='20250717'
          and login_date_last=date_add('20250717',-7)
    )churn
        join
    (
        select
...
select '20250717' dt,
       ...
       datediff('20250717', login_date_first) retention_day,
       sum(if(login_date_last = '20250717', 1, 0)) retention_count,
       count(*) new_user_count,
       cast(sum(if(login_date_last = '20250717', 1, 0)) / count(*) * 100 as decimal(16, 2)) retention_rate
from (
         select user_id,
                login_date_last,
                login_date_first
         from gmall_dws.dws_user_user_login_td
         where dt = '20250717'
           and login_date_first >= date_add('20250717', -7)
           and login_date_first < '20250717'
     ) t1
...
union
select '20250717' as dt,
       recent_days,
       sum(if(login_date_first >= date_add('20250717', -recent_days + 1), 1, 0)) new_user_count,
       count(*) active_user_count
from gmall_dws.dws_user_user_login_td lateral view explode(array(1, 7, 30)) tmp as recent_days
where dt = '20250717'
  and login_date_last >= date_add('20250717', -recent_days + 1)
...
where dt='20250717'
  and order_date_first>=date_add('20250717',-recent_days+1)
group by recent_days;
...
where dt in ('99991231','20250717')
  and payment_date_id='20250717';

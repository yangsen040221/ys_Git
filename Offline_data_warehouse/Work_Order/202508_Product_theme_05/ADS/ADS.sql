...
(`monitor_product_id`,
 ...
 `conversion_count`,
 `stat_date`)
SELECT t.main_product_id AS monitor_product_id,
...
FROM (
         ...
         GROUP BY main_product_id
         ORDER BY total_guide_visits DESC LIMIT 10) t
...
select *
from ads_monitor_product_relation;
...
ON s.main_product_id = p.product_id
ORDER BY s.visit_to_cart_rate DESC;



select *
from ads_product_sequence_analysis;
